...
export const ALL_PRODUCTS = Object.values(PRODUCTS).sort((a, b) => a.title.localeCompare(b.title));
/** Array of active/enabled products (status = 'active' or 'beta'). */
export const ENABLED_PRODUCTS = ALL_PRODUCTS.filter((p) => p.status === 'active' || p.status === 'beta');
/** Array of featured products. */
export const FEATURED_PRODUCTS = ALL_PRODUCTS.filter((p) => p.featured && (p.status === 'active' || p.status === 'beta'));
/** Array of coming-soon products. */
export const COMING_SOON_PRODUCTS = ALL_PRODUCTS.filter((p) => p.status === 'coming-soon' || p.status === 'planned');
...
export const PRODUCT_ICONS = Object.fromEntries(ALL_PRODUCTS.map((p) => [p.id, p.icon]));
