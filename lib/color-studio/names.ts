 ...
 *
 * Based on extended web color names with hex values.
 */

import type { NamedColor, RGB } from './types';
...
NAMED_COLORS.forEach((c) => {
  nameMap.set(c.name.toLowerCase(), c.hex);
  hexToNameMap.set(c.hex.toLowerCase(), c.name);
});

/** Find the closest named color for a hex string, or null. */
export function findColorName(hex: string): string | null {
  return hexToNameMap.get(hex.toLowerCase()) ?? null;
}

/** Get hex for a named color (case-insensitive). */
export function getNamedColorHex(name: string): string | null {
  return nameMap.get(name.toLowerCase()) ?? null;
}

/** Search named colors by substring. */
export function searchNamedColors(query: string): NamedColor[] {
