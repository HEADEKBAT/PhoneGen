...
import { Scan, Box, Package, ShoppingCart, Code, Tag, type LucideIcon, ArrowRight } from 'lucide-react';
import type { UseCase } from '@/lib/config/barcode';
...
const ICON_MAP: Record<string, LucideIcon> = {
  Scan, Box, Package, ShoppingCart, Code, Tag,
};
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 py-16 sm:py-20">
        <div className="text-center mb-12">
          ...
          <p className="mt-2 text-sm text-muted-foreground max-w-md mx-auto">
            {subtitle || 'One click to generate the right barcode for any scenario'}
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {useCases.map((uc) => {
            const Icon = ICON_MAP[uc.icon] || Scan;
            return (
              <Link
                key={uc.id}
                href={`/${locale}/barcode-generator/tool?type=${uc.preset}`}
